...
def eval_ood_detector(args):
    ...
    activation_log = np.concatenate(activation_log, axis=0)#join a sequence of arrays along an existing axis.
    print(activation_log,"jump out of the loop the estimate the activation_log")
    # from scipy import stats
    # stats.percentileofscore(activation_log.flatten(), 3.5)
    print(f"\nTHRESHOLD at percentile {97} is:")
    print(np.percentile(activation_log.flatten(), 97))
