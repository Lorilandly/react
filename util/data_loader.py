...
def get_loader_in(args, config_type='default', split=('train', 'val')):
    ...
    if args.in_dataset == "CIFAR-10":
        # Data loading code
        if 'train' in split:
            trainset = torchvision.datasets.CIFAR10(root='/data1/TL/data/cifar-10', train=True, download=True, transform=config.transform_train)
            train_loader = torch.utils.data.DataLoader(trainset, batch_size=config.batch_size, shuffle=True, **kwargs)
        if 'val' in split:
            valset = torchvision.datasets.CIFAR10(root='/data1/TL/data/cifar-10', train=False, download=True, transform=transform_test)
            val_loader = torch.utils.data.DataLoader(valset, batch_size=config.batch_size, shuffle=True, **kwargs)
        num_classes = 10
    elif args.in_dataset == "CIFAR-100":
        # Data loading code
        if 'train' in split:
            trainset = torchvision.datasets.CIFAR100(root='/data1/TL/data/cifar-100', train=True, download=True, transform=config.transform_train)
            train_loader = torch.utils.data.DataLoader(trainset, batch_size=config.batch_size, shuffle=True, **kwargs)
        if 'val' in split:
            valset = torchvision.datasets.CIFAR100(root='/data1/TL/data/cifar-100', train=False, download=True, transform=config.transform_test)
            val_loader = torch.utils.data.DataLoader(valset, batch_size=config.batch_size, shuffle=True, **kwargs)
        num_classes = 100
    elif args.in_dataset == "imagenet":
        root = '/data1/TL/data/ImageNet/'
        # Data loading code
        if 'train' in split:
...
def get_loader_out(args, dataset=['SVHN','LSUN','LSUN_resize','iSUN','dtd','places365'], config_type='default', split=('train', 'val')):
    ...
    if 'val' in split:
        ...
        if val_dataset == 'SVHN':
            val_ood_loader = torch.utils.data.DataLoader(torchvision.datasets.SVHN('datasets/ood_datasets/svhn/', split='test', transform=transform_test, download=False),
            ...
            val_ood_loader = torch.utils.data.DataLoader(torchvision.datasets.CIFAR100(root='/data1/TL/data/cifar-100', train=False, download=True, transform=transform_test),
                                                       batch_size=batch_size, shuffle=True, num_workers=2)
