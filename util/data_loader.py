...
#this makes some changes, and original is |def get_loader_out(args, dataset=(' '), config_type='default', split=('train', 'val')):
    print(dataset[1],'first in datasets')
    config = EasyDict({
        "default": {
            'transform_train': transform_train,
            'transform_test': transform_test,
    ...
    if 'val' in split:
        ...
        if val_dataset == 'SVHN':
            ...
            val_ood_loader = torch.utils.data.DataLoader(
                torchvision.datasets.ImageFolder(os.path.join('./data1/TL/data/ImageNet', 'val'), config.transform_test_largescale),
                batch_size=config.batch_size, shuffle=True, **kwargs)
        else:
